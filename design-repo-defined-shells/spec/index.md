...
- `global_shell: Option<String>` — full nix flake ref (e.g.
  `~/.config/cast/nix#default`, `.#ai`, `/abs/path#shell`,
  `github:org/repo#shell`). Unset = no global layer. Verbatim pass-through.
...
- `use_global_flake: bool` (default `true`) and `use_project_flake: bool`
...
Per-agent shell selection remains reachable via `CAST_GLOBAL_SHELL` env
...
(numtide substituter/key + `global_shell` set to the tilde ref
...
- No global shell configured: docker exec failure surfaces as-is; no
  cast-side notice (user will evaluate the raw failure mode later).
- The "loading global nix devshell" stderr announcement re-gates on the
  effective config (ref set AND switch true), preserving the headless
  JSON contract.

## Scope

- Config schema change and env override surface (`CAST_GLOBAL_SHELL`,
  `CAST_PROJECT_SHELL`, `CAST_USE_GLOBAL_FLAKE`,
  `CAST_USE_PROJECT_FLAKE`; `CAST_USE_FLAKE`/`CAST_USE_FLAKE_PATH` die).
